...
async def DeleteUser(id: int,status:bool)->str:
    deleteQuery=conn.execute(users.update().where(users.c.id == id).values({"status":status}))
...
async def UpdateUser(id:int)->str:
    updateQuery=conn.execute(users.update().where(users.c.id == id).values())
    return 'actualizado'

@user_route.get("/user/{id}")
async def GetUser(id:int)->str:
    getQuery=conn.execute(users.select().where(users.c.id==id)).fetchall()
    return getQuery  
